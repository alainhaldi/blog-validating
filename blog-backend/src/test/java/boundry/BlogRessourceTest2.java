...
// @QuarkusTest
// @TestInstance(Lifecycle.PER_CLASS) // Sicherstellen, dass die Tests in der
// gleichen Instanz ausgeführt werden
// @TestMethodOrder(MethodOrderer.OrderAnnotation.class) // Ermöglicht, dass die
// Tests in der richtigen Reihenfolge
// // ausgeführt werden
...
// @Test
// @Order(1) // Wird als erstes ausgeführt
...
// .get("/blogs")
// .then()
// .statusCode(Status.OK.getStatusCode()) // Optionales prüfen des Status-Codes
// .extract().body().asString(); // Auslesen des Response-Bodys

// // JSON-Array aus dem Response-Body erstellen mit Hilfe von JSON-P
// JsonArray jsonArray = Json.createReader(new
// StringReader(responseBody)).readArray();
...
// JsonArray jsonBlogArray = Json.createReader(new
// StringReader(responseBody)).readArray();
...
// JsonObject jsonBlogObject = Json.createReader(new
// StringReader(responseBody)).readObject();

// Assertions.assertEquals(NEW_BLOG_TITLE, jsonBlogObject.getString("title"));
// Assertions.assertEquals(NEW_BLOG_CONTENT,
// jsonBlogObject.getString("content"));
// }
// }
